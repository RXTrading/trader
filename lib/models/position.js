...
class Position extends Model {
  ...
  findClosedEntryOrders () {
    return this.orders.filter(order => {
      const side = this.type === Position.types.LONG ? OrderOptions.sides.BUY : OrderOptions.sides.SELL

      return order.side === side && order.closedAt !== undefined
    })
  }

  findClosedExitOrders () {
    return this.orders.filter(order => {
      const side = this.type === Position.types.LONG ? OrderOptions.sides.SELL : OrderOptions.sides.BUY
