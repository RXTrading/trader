...
class PositionManager {
  ...
  async close (params = {}) {
    ...
    if (position.orders.length === 0) {
      const closedAt = params.timestamp ? params.timestamp : new Date()
      ...
      openOrders.forEach(order => {
        this.#trader.exchange.cancelOrder({ id: order.foreignId, ..._.omit(params, 'id') })
  ...
  async #evaluatePosition (position, options = {}) {
    ...
    const positionClosing = position.status === Position.statuses.CLOSING

    position.orders.forEach(order => this.#evaluateOrder(order))

    if (!positionClosing && position.exits.length > 0 && this.#ordersAreFilled(entryOrders)) {
      await this.#evaluateExitOrders(position, options)
    }

    if (this.#shouldPositionBeClosed(position)) {
      const allClosedAts = position.orders.map(order => new Date(order.closedAt).getTime())
      const closedAt = new Date(Math.max(...allClosedAts))
  ...
  #ordersAreFilled (orders) {
    return orders.every(order => [Order.statuses.FILLED, Order.statuses.CANCELLED].includes(order.status))
  }
  ...
  #shouldPositionBeClosed (position) {
    ...
    const entryOrders = position.findClosedEntryOrders(position)
    const offsetOrders = position.findClosedExitOrders(position)
